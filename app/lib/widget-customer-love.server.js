...
const WIDGET_ID = "customers-love-page";

export async function widgetCustomerLoveLoader({ session, billing }) {
...
export async function widgetCustomerLoveAction({ request, session }) {
  const shop = normalizeShopDomain(session.shop);
  ...
  if (!hasProAccess(planStatus)) {
    return { ok: false, publishError: "Customer's Love Page requires a Pro plan." };
  }
