...
const WIDGET_ID = "video-reviews-slider";

export async function widgetVideoSliderLoader({ session, billing }) {
...
export async function widgetVideoSliderAction({ request, session }) {
  const shop = normalizeShopDomain(session.shop);
  ...
  if (!hasProAccess(planStatus)) {
    return { ok: false, publishError: "Video Reviews Slider requires a Pro plan." };
  }
