...
export async function widgetQALoader({ session, billing }) {
...
export async function widgetQAAction({ request, session }) {
  const shop = normalizeShopDomain(session.shop);
  ...
  if (!hasProAccess(planStatus)) {
    return { ok: false, publishError: "Q&A widget requires a Pro plan." };
  }
